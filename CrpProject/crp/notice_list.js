...
export default class SearchActivity extends Component {
    ...
    _renderTopItem = (itemData, index) => {
        return <TouchableOpacity key={index} activeOpacity={1} onPress={() => this.selectMenu(itemData, index)}>
            ...
            <View style={{ flexDirection: 'column' }}>
                ...
                <Text style={[{ height: 48, width: 60 }, styles.top_name]}>{itemData.item.name}</Text>
                {this.selectIcon(itemData.item.select)}
            </View>
        </TouchableOpacity >
    ...
    selectIcon(isSelect) {
        if (isSelect) {
            ...
            return <View style={{ height: 2, width: 60, backgroundColor: '#ff9602' }} />;
        }
